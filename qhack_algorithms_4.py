...
def circuit(indices):
    ...
    grover_op = grover_operator(indices)

    for i in target_wires:
        qml.Hadamard(i)

    qml.QuantumPhaseEstimation(grover_op, target_wires, estimation_wires)
    # QHACK #
...
def number_of_solutions(indices):
    ...
    # QHACK #
    probs2 = circuit(indices)
    print(probs2)
    val = np.amax(probs2)
    index = np.where(probs2==val)[0][0]
    print(index)
    theta = index*np.pi/8
...
def relative_error(indices):
    ...
    # QHACK #
    estimate = number_of_solutions(indices)
    print(estimate)
    true_num = len(indices)
    print(true_num)
    rel_err = (estimate-true_num)/true_num*100
