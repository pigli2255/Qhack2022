...
def is_particle_preserving(circuit, n):
    ...
    """
    # QHACK #
    basisStates = basis_states(n)

    outputStates = list()
    ret2 = binary_list(0, n)
    for state in basisStates:
        output = circuit(state)
        ret = [i for i in range(len(output)) if abs(output[i]) > 0]
        for index in ret:
            ret2 = np.array(ret2) + np.array((np.abs(output[index])**2)*binary_list(index, n))
        outputStates.append(ret2)

    #print(basisStates)
    n_ins = np.sum(basisStates, axis=1)
    n_outs = np.sum(outputStates, axis=1)
