...
def superdense_coding(bits, alpha):
    ...
    # QHACK #
    entangelingMatrix = np.array([[np.cos(alpha), np.sin(alpha)],[np.sin(alpha), -np.cos(alpha)]])
    # Prepare entangled state here
    qml.QubitUnitary(entangelingMatrix, wires=0)
    qml.CNOT(wires=[0,1])
    ...
    if bits==0:
        ...
        print("The given message is invalid. The message needs to be 0,1,2 or 3")
