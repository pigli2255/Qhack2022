...
def hamiltonian_coeffs_and_obs(graph):
    ...
    obs_singlebody = [qml.PauliZ(i) for i in range(num_vertices)] + [qml.Identity(i) for i in range(num_vertices)]
    coeffs_single = [-1/2 for i in range(2*num_vertices)]

    # Create interaction terms
    obs_twobody = []
    coeffs_two = []
    i_ind, j_ind = np.where(E==True)

    for i,j in zip(i_ind,j_ind):
        obs_twobody.append(qml.PauliZ(i)@qml.PauliZ(j))
        obs_twobody.append(qml.Identity(i) @ qml.Identity(j))
        coeffs_two.append(u / 4)
        coeffs_two.append(u / 4)
        obs_twobody.append(qml.PauliZ(i))
        obs_twobody.append(qml.PauliZ(j))
        coeffs_two.append(u/4)
        coeffs_two.append(u / 4)

    coeffs = coeffs_single + coeffs_two
    obs = obs_singlebody + obs_twobody
    # QHACK #
...
def variational_circuit(params, num_vertices):
    ...
    def circuit(params, num_vertices):

        for j in range(3):
            for i in range(num_vertices):
                qml.Rot(params[j, i, 0], params[j, i, 1], params[j, i, 2], wires=i)
                if i < num_vertices - 1:
                    qml.CNOT(wires=[i, i + 1])
                else:
                    qml.CNOT(wires=[i, 0])

    return circuit(params, num_vertices)
    # QHACK #
...
def train_circuit(num_vertices, H):
    ...
    epochs = 300
    params = np.random.rand(3, num_vertices, 3, requires_grad=True)
