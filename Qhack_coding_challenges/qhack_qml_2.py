...
def distance(A, B):
    ...
    len_data=len(A)+len(B)
    nqubits = int(np.ceil(np.log2(len_data)))

    dev = qml.device("default.qubit", wires=nqubits+1)
    @qml.qnode(dev)
    def circuit(A, B):
        qml.AmplitudeEmbedding(features=[A[0]*B[0], A[0]*B[1], A[1]*B[0], A[1]*B[1]], wires=range(1, nqubits+1), normalize=True, pad_with=0.)
        qml.Hadamard(0)
    ...
    overlap = 2*circuit(A, B)[0]-1
    d = np.sqrt(2*(1-np.sqrt(overlap)))
    return d
